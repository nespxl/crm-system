...
function CreateChangeModalWindow(title: string = 'Изменить данные', lastButton: string = 'Удалить клиента') {
	...
	btnDeleteForm.addEventListener('click', (e: any) => {
		e.preventDefault();
		containerMain?.append(deleteItemTable.externalContModal);
  	});

		internalContModal.addEventListener('click', (e: any) => {
	...
	closeIcon.addEventListener('click', () => {
		...
		window.scrollTo(0, scrollPosition);
		html?.classList.remove('is'); 
		containerMain?.removeChild(externalContModal);
	});

  	function createSelect(type: string = '', value: string = '') {
		...
		} else eraseInput.classList.remove('clients__modal-erase-input-true');    
